function NMF_RI_Photometry
    ...
    function run_nmf_ri()
        ...
        try
            %% 1. Preprocessing
            % 각 timepoint spectrum별 baseline offset 제거
            Y_prep = Y - min(Y, [], 1);
    ...
    function download_results()
        ...
        Tbl = table((0:T-1)', appState.H(1,:)', appState.H(2,:)', ...
            'VariableNames', {'TimePoint', name1, name2});
        writetable(Tbl, fullfile(path, file));

        specFile = strrep(fullfile(path, file), '.csv', '_spectra.csv');
        SpecTbl = table(appState.wavelengths(:), ...
            appState.A(:,1), appState.A(:,2), ...
            'VariableNames', {'Wavelength_nm', name1, name2});
        writetable(SpecTbl, specFile);

        reconFile = strrep(fullfile(path, file), '.csv', '_reconstruction.csv');
        Y_recon = appState.A * appState.H;
        ReconTbl = array2table(Y_recon');
        ReconTbl.Properties.VariableNames = matlab.lang.makeValidName( ...
            "WL_" + string(round(appState.wavelengths(:)')) );
        writetable(ReconTbl, reconFile);

        txtOutStatus.Text = sprintf('저장 완료: %s', fullfile(path, file));
    end
end
...
function em = loadFPbaseEmission(csvFile, wl_target)
    ...
    for i = 2:length(colNames)
        ...
        if contains(name, 'em') && ~contains(name, 'ex') && ~contains(name, '2p')
            emIdx = i;
            break;
        end
    end
    ...
    if isempty(emIdx)
        error('emission 컬럼을 찾을 수 없습니다: %s', csvFile);
    end
