function Linear_Spectral_Unmixing_Algorithm_v2
    fig = uifigure('Name','Linear Spectral Unmixing Model v2','Position',[100,100,1150,720]);
    ...
    %% Sidebar Panel 1
    lblPanel1 = uilabel(sidebar,'Position',[10,635,300,22],'Text','Panel 1: Raw TXT Unmixing','FontAngle','italic');

    lblMixed = uilabel(sidebar,'Position',[10,605,300,22],'Text','Upload mixed raw TXT file');
    btnMixed = uibutton(sidebar,'Position',[10,580,300,24],'Text','Browse mixed TXT...', ...
        'ButtonPushedFcn',@(btn,e) uigetfile_callback('mixed'));
    txtMixedName = uilabel(sidebar,'Position',[10,555,300,22],'Text','No file selected','FontColor',[0.5 0.5 0.5]);

    lblName1 = uilabel(sidebar,'Position',[10,525,130,22],'Text','Component 1 Name:');
    txtName1 = uieditfield(sidebar,'text','Position',[145,525,160,22],'Value','');
    ...
    btnRef1 = uibutton(sidebar,'Position',[10,465,300,24],'Text','Browse Component 1 refs TXT...', ...
        'ButtonPushedFcn',@(btn,e) uigetfile_callback('ref1'));
    txtRef1Name = uilabel(sidebar,'Position',[10,440,300,22],'Text','No ref selected','FontColor',[0.5 0.5 0.5]);

    lblName2 = uilabel(sidebar,'Position',[10,400,130,22],'Text','Component 2 Name:');
    txtName2 = uieditfield(sidebar,'text','Position',[145,400,160,22],'Value','');
    ...
    btnRef2 = uibutton(sidebar,'Position',[10,340,300,24],'Text','Browse Component 2 refs TXT...', ...
        'ButtonPushedFcn',@(btn,e) uigetfile_callback('ref2'));
    txtRef2Name = uilabel(sidebar,'Position',[10,315,300,22],'Text','No ref selected','FontColor',[0.5 0.5 0.5]);

    lblWavelength = uilabel(sidebar,'Position',[10,280,300,22], ...
        'Text','Wavelength range: 450–700 nm fixed','FontWeight','bold');

    btnRun = uibutton(sidebar,'Position',[10,240,300,32],'Text','Run', ...
        'ButtonPushedFcn',@(btn,e) run_analysis());

    lblOutput = uilabel(sidebar,'Position',[10,195,300,22],'Text','Output file name:');
    txtOutputName = uieditfield(sidebar,'text','Position',[10,170,300,22],'Value','');

    btnDownload = uibutton(sidebar,'Position',[10,130,300,30],'Text','Download dataset', ...
        'ButtonPushedFcn',@(btn,e) download_data());

    %% Sidebar Panel 2
    lblPanel2 = uilabel(sidebar,'Position',[10,635,300,22],'Text','Panel 2: Timepoint Fit Check', ...
        'FontAngle','italic','Visible','off');

    lblTimePoint = uilabel(sidebar,'Position',[10,605,300,22],'Text','Enter time point:', ...
        'Visible','off');
    ...
    txtInfo = uitextarea(tab2,'Position',[40,40,680,180],'Editable','off','FontName','FixedWidth');
    ...
    function run_analysis()
        ...
        [wavelengths,mixed_spectra,~] = readMixedTxt(appState.mixed_file);
        ...
        uialert(fig,'분석 완료. Download dataset으로 저장할 수 있습니다.','완료');
    end
    ...
    function download_data()
        ...
        % reference spectrum 파일 이름
        [~, savedBaseName, ~] = fileparts(file);
        refFile = fullfile(path, [savedBaseName '_reference_spectra.txt']);
