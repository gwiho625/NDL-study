function GH_NMF_RI_Photometry
    ...
    fig = uifigure('Name', 'NMF-RI Blind Unmixing — Fiber Photometry', ...
    ...
    appState.gcampFile = '';
    appState.jrgecoFile = '';
    appState.Y = [];           % [W x T] raw cropped mixed spectra
    appState.Y_prep = [];      % [W x T] preprocessed non-negative spectra
    appState.wavelengths = [];
    appState.A = [];           % [W x 2] resolved spectra
    appState.H = [];           % [2 x T] concentration profiles
    appState.A0 = [];          % [W x 2] initial spectra
    appState.nIter = 0;
    ...
    uilabel(tab1, 'Position', [30, 695, 400, 22], ...
        'Text', '① 혼합 스펙트럼 시계열 (raw TXT)', 'FontWeight', 'bold');
    ...
    title(axInit, '초기 스펙트럼 A0 미리보기 (FPbase 이론값)');
    ...
    axLOF = uiaxes(tab2, 'Position', [450, 300, 730, 400]);
    ...
    title(axRecon, 'Observed vs Reconstructed (Preprocessed, Timepoint 1)');
    ...
    uibutton(tab3, 'Position', [470, 56, 160, 30], ...
        'Text', 'Download CSV', ...
        'ButtonPushedFcn', @(b,e) download_results());

    txtOutStatus = uilabel(tab3, 'Position', [650, 60, 500, 30], ...
        'Text', '', 'FontColor', [0.3 0.3 0.3], 'WordWrap', 'on');

    %% ===================== Callback functions =====================

    function browse_mixed()
        [file, path] = uigetfile({'*.txt', 'TXT files'}, ...
            'Select mixed raw TXT');
    ...
    function load_data()
        ...
        [wl, spectra, ~] = readMixedTxt(appState.mixedFile);
        ...
        txtDataStatus.Text = sprintf( ...
            '로딩 완료: Y = [%d x %d] (파장 x 시점), 파장 %.0f~%.0f nm', ...
            W, T, wl_min, wl_max);
    end
    ...
    function plot_results()
        ...
        cla(axConc);
        hold(axConc, 'on');
        plot(axConc, 1:T, H(1,:), 'g-', 'LineWidth', 1.0, 'DisplayName', name1);
        plot(axConc, 1:T, H(2,:), 'r-', 'LineWidth', 1.0, 'DisplayName', name2);
        hold(axConc, 'off');
        ...
        plot(axRecon, wl, Yplot(:,1), 'k-', 'LineWidth', 1.5, ...
            'DisplayName', 'Observed preprocessed (t=1)');
        plot(axRecon, wl, Y_recon(:,1), 'b--', 'LineWidth', 1.5, ...
            'DisplayName', 'Reconstructed (t=1)');
        hold(axRecon, 'off');
    ...
    function download_results()
        ...
        [file, path] = uiputfile('*.csv', '결과 저장', ...
            [txtOutName.Value, '.csv']);
        ...
        Tbl = table( ...
            ...
            appState.H(1,:)', ...
            appState.H(2,:)', ...
            'VariableNames', {'TimePoint', name1, name2});

        writetable(Tbl, outFile);

        %% 2) Resolved spectra A 저장
        specFile = replace_file_suffix(outFile, '_spectra.csv');
        ...
        %% 3) Reconstruction 저장
        % wavelength를 변수명으로 직접 쓰지 않고 WL_0001, WL_0002 ... 형식 사용
        % 실제 wavelength 값은 wavelength_map 파일에 따로 저장
        reconFile = replace_file_suffix(outFile, '_reconstruction.csv');

        Y_recon = appState.A * appState.H;   % [W x T]
        ReconTbl = array2table(Y_recon');    % [T x W]
        ...
        ReconTbl.Properties.VariableNames = cellstr(reconVarNames);

        ReconTbl = addvars( ...
            ReconTbl, ...
            (0:T-1)', ...
            'Before', 1, ...
            'NewVariableNames', 'TimePoint');

        writetable(ReconTbl, reconFile);

        %% 4) Reconstruction wavelength map 저장
        mapFile = replace_file_suffix(outFile, '_reconstruction_wavelength_map.csv');
        ...
        observedFile = replace_file_suffix(outFile, '_observed_preprocessed.csv');
        ...
        ObsTbl = array2table(Y_obs');
        ObsTbl.Properties.VariableNames = cellstr(reconVarNames);
        ...
        uialert(fig, sprintf([ ...
            ...
            outFile, specFile, reconFile, mapFile, observedFile), ...
            'CSV 저장 완료');
    end
end
...
function newFile = replace_file_suffix(filePath, suffix)
    [folder, baseName, ~] = fileparts(filePath);
    newFile = fullfile(folder, [baseName suffix]);
end
